...
| POST | `/shifts/break/start` | Bearer token | Starts a lunch break on the caller's active shift — 404 if no active shift, 409 if a break is already open |
| POST | `/shifts/break/end` | Bearer token | Ends the caller's open break — 404 if none in progress |
...
**Lunch break built (2026-08-27)** — a scoped-down version of the requirements doc's "Automated Break Management" future-dev section: staff-initiated start/end only, no admin-scheduled windows or reminders yet. New `Break` model, separate from `Shift` on purpose — going to lunch no longer means stopping (or never stopping) the whole shift. One open break per shift at a time; ending the shift auto-closes any open break. Break time is subtracted from every worked-hours total (`ReportsService.attendance()`, `adminDashboard()`, and the Sheets "Hours Worked" export) — it's unpaid time, not time on task. Also fixed a real latent bug found while testing this: `SheetsService.ensureTab()` only wrote a tab's header once, at creation — adding a column (the new "Break Minutes") to an already-existing tab left old and new rows misaligned under a stale header. Fixed to always re-sync row 1 to the current header (via `update`, not `append`, so it overwrites row 1 rather than stacking a duplicate). Backend e2e: 52/52 passing, `tsc` clean. Verified live: full start-shift → start-break → end-break → end-shift flow via curl, and the Sheets export read back to confirm both the header fix and the new columns.

**Google Sheets export built (2026-08-24) — `src/sheets/`.** Optional, off by default (needs `GOOGLE_SHEETS_SPREADSHEET_ID` + either a local `google-service-account.json` key file or the `GOOGLE_SHEETS_CLIENT_EMAIL`/`GOOGLE_SHEETS_PRIVATE_KEY` env vars for the hosted deploy — see `.env.example`). When configured: shifts write a row to an "Attendance" tab on `end`, receptions write a row to a "Reception" tab on `complete` (matching the requirements doc's own "documented in a Google Sheet" / "all recorded in a Google sheet" lines), and `POST /reports/export-to-sheets` (admin/management) appends a timestamped snapshot of every report to 3 more tabs. Tabs are auto-created with a header row the first time anything's written to them. **Never throws** — a Sheets outage or missing config can't break the feature that triggered the export, only gets logged. Verified for real, not just via tests: ran the actual flow against the real configured spreadsheet and read the rows back via the Sheets API to confirm they landed correctly — found and fixed one real issue in the process (timestamps used the server's ambient locale, which would've looked different on Render than on this dev machine; pinned to an explicit format).

**Deployment path chosen (2026-08-24): Render (backend) + Neon (Postgres) + Cloudinary (photos), all free tiers, for an initial no-cost company pilot.** `render.yaml` at the repo root is a deploy blueprint — Render reads it to configure the build/start commands automatically instead of manual dashboard setup. Two things had to change in the app itself to make hosting off this laptop actually work:
- `src/uploads/` now uses Cloudinary when `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET` are all set (a hand-written Multer storage engine — `multer-storage-cloudinary`'s latest release only supports Cloudinary SDK v1, incompatible with the v2 SDK this project uses), and **falls back to local disk when they aren't** — so local dev and the e2e suite need zero Cloudinary setup and are unaffected. This matters because Render's free tier has no persistent disk: anything saved locally is lost on every restart, which would have meant Reception/Seller Stock's damage-evidence and label photos silently disappearing.
- Chose Neon over Render's own free Postgres because Render's free database **auto-deletes after 90 days** — too close to the "prove this is worth paying for" trial window to risk.

See `docs/architecture.md`'s 2026-08-24 entry for the full account-setup walkthrough (Render/Neon/Cloudinary are all account-bound steps, done by the user, not scriptable from here) and free-tier trade-offs (cold-start delay on Render's free web service, Neon's sleep-on-idle behavior).

**Live since 2026-08-26** at a Render-assigned URL, confirmed via real requests against the actual deployed instance (health check + a real login against the real Neon database), not just the dashboard's status label. Getting there needed one real fix — see `docs/architecture.md`'s dated entry: `nest build` was nesting compiled output under `dist/src/main.js` instead of the flat `dist/main.js` the start command expects, because the build was compiling the sibling `prisma/` seed scripts alongside `src/`. Fixed in `tsconfig.build.json`. This had been latent in every local build; it only surfaced because local dev never actually executes the literal compiled file the way a real deploy does.
