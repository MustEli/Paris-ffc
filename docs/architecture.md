...
- **Real hosting chosen and prepped (2026-08-24): Render + Neon + Cloudinary, all free tiers.** Context: this app is being built for a company evaluating it during an unpaid trial — "no unnecessary costs until they're sure it's valuable" — so the hosting choice had to be free for at least ~3 months, not just cheap. Decided:
  ...
  **Not yet done: nothing has actually been deployed** — this is all prep, verified locally (tsc clean both packages, e2e 47/47, real android+ios Metro bundles build) but not yet exercised against a real Render/Neon/Cloudinary account, since those don't exist yet. First real deploy should be expected to need a round of troubleshooting together — PaaS build environments (npm's devDependency-pruning behavior under `NODE_ENV=production`, exact Prisma/monorepo path resolution) are the kind of thing that often needs one real attempt to shake out, not something fully verifiable without the actual account.
