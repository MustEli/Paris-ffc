...
- **Not yet device-verified:** this integration (mobile calling the real backend) has only been checked via `tsc` + real bundle builds + the backend's own e2e suite — not actually tried from the phone yet. One concrete risk found before testing: this machine's Wi-Fi is a **Public**-profile network (matches the phone hotspot) with no firewall rule allowing inbound traffic to the backend's port — Windows may silently block the phone from reaching it even though `localhost` and this-machine-to-its-own-LAN-IP both work. See the firewall note below.
